...
function selectAnswer(choice) {
  ...
  for (let button of buttons) {
      button.disabled = true; // 답변 선택 후 버튼 비활성화
      if (button.innerText === correctAnswer) {
          button.classList.add("correct"); // 정답 버튼 초록색으로 표시
      }
  }
...
function showResult() {
  ...
  if (score === quizData.length) {
      message = "모든 문제를 정확히 맞추셨습니다! 축하합니다!";
  } else if (score >= 15) {
      message = "잘 하셨습니다! 조금만 더 분발하세요!";
  } else if (score >= 10) {
      message = "보통 수준입니다. 더 노력해 보세요.";
  } else {
      message = "공부가 필요합니다. 화이팅!";
  }
